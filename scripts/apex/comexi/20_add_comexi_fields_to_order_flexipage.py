#!/usr/bin/env python3
"""Añade las secciones COMEXI a la pestaña Detalles de RLM_Order_Record_Page.

Gemelo de `18_add_comexi_fields_to_contract_flexipage.py`, con una diferencia que
conviene recordar: aquí sí funciona tocar la flexipage. La Lightning Record Page
del pedido construye la pestaña Detalles con `flexipage:fieldSection`, así que los
campos nuevos se declaran aquí. En el contrato no: aquella página monta el detalle
con `force:detailPanel` y lee el page layout, y por eso hubo que parchear el layout.
Comprueba siempre cuál de los dos casos tienes antes de dar por hecho el otro.
...
SRC = REPO / "unpackaged/post_ux/flexipages/RLM_Order_Record_Page.flexipage-meta.xml"
DST = REPO / "unpackaged/post_comexi/flexipages/RLM_Order_Record_Page.flexipage-meta.xml"

# La pestaña Detalles es el facet `detailTabContent`.
DETAIL_REGION = "detailTabContent"
# Detrás de Order Information, para que las secciones COMEXI queden arriba y no
# enterradas bajo System Information.
ANCHOR_IDENTIFIER = "flexipage_fieldSection"

SECTIONS = [
    (
        "COMEXI: Origen del pedido",
        [
            "COMEXI_Source_Case__c",
            "COMEXI_Source_Opportunity__c",
            "COMEXI_Generated_By_Advisor__c",
        ],
...
def field_identifier(api_name):
    return "comexiO" + re.sub(r"[^A-Za-z0-9]", "", api_name.replace("COMEXI_", "")) + "Field"
...
def main():
    ...
    detail = next(
        (
            r
            for r in root.findall(q("flexiPageRegions"))
            if (r.find(q("name")) is not None and r.find(q("name")).text == DETAIL_REGION)
        ),
        None,
    )
    if detail is None:
