#!/usr/bin/env python3
"""Añade las secciones COMEXI a la pestaña Detalles de RLM_Contract_Record_Page.

Mismo problema que en Account y en Quote: la Lightning Record Page del contrato
no renderiza el detalle del page layout, construye la pestaña Detalles con
`flexipage:fieldSection` de campos fijos. Tocar solo el Layout no hace visible
ningún campo nuevo, hay que inyectarlos aquí.
...
SRC = REPO / "unpackaged/post_ux/flexipages/RLM_Contract_Record_Page.flexipage-meta.xml"
DST = REPO / "unpackaged/post_comexi/flexipages/RLM_Contract_Record_Page.flexipage-meta.xml"

# La pestaña Detalles es el facet `detailTabContent` (lo referencia el tab con
# identifier `detailTab`).
DETAIL_REGION = "detailTabContent"
# Insertamos detrás de Contract Information para que las cuatro secciones
# COMEXI queden arriba y no enterradas bajo System Information.
ANCHOR_IDENTIFIER = "flexipage_fieldSection"

SECTIONS = [
    (
        "COMEXI: Origen del contrato",
        [
    ...
    (
        "COMEXI: Condiciones contractuales",
        [
            "COMEXI_Contract_Value__c",
            "COMEXI_Down_Payment_Pct__c",
            "COMEXI_Down_Payment_Amount__c",
        ],
        [
            "COMEXI_Payment_Terms__c",
            "COMEXI_Warranty_Months__c",
            "COMEXI_Warranty_Start_Event__c",
        ],
    ),
]
...
def field_identifier(api_name):
    return "comexiC" + re.sub(r"[^A-Za-z0-9]", "", api_name.replace("COMEXI_", "")) + "Field"
