...
### `activate_comexi_docgen_template`

**Description:** Activate the latest version of each COMEXI_ DocumentTemplate. DocumentTemplates always land inactive from a deploy regardless of isActive in the .dt-meta.xml, and the Document Template picklist in the Create Proposal modal only lists templates that are active and carry usageType Revenue_Lifecycle_Management, so without this step COMEXI_Propuesta_Retrofit exists in the org but cannot be picked. Same for COMEXI_Contrato_Retrofit, which carries usageType Contract_Lifecycle_Management and feeds both the Generate Contract modal and COMEXI_ContractDocQueueable. The base equivalent, scripts/apex/activateDocgenTemplates.apex, filters Name LIKE 'RLM_%' and never touches the COMEXI ones. Every deploy lands a new version, so the task also archives the previous one - two active versions with the same name show up twice in the picklist. Idempotent - no DML when they are already active.

**Class:** `cumulusci.tasks.apex.anon.AnonymousApexTask`

**Options:**

- `path`: `scripts/apex/comexi/17_activate_comexi_docgen_template.apex`
...
**Description:** Deploy COMEXI retrofit demo delta from unpackaged/post_comexi: 57 custom fields (Account.COMEXI_Has_Representative__c / COMEXI_Representative_Commission_Pct__c, Asset.COMEXI_Machine_Model__c / COMEXI_Install_Year__c / COMEXI_Installed_Config__c / COMEXI_Line__c, Opportunity.COMEXI_Source_Case__c plus 18 retrofit fields on Opportunity grouped in four Details sections - Machine (COMEXI_Target_Asset__c lookup, COMEXI_Machine_Model__c, COMEXI_Machine_Age_Years__c, COMEXI_Obsolete_Component__c), Viability (COMEXI_Viability_Status__c, COMEXI_Requires_Engineering__c, COMEXI_Validated_By__c, COMEXI_Advisor_Confidence__c, COMEXI_Advisor_Run_At__c, COMEXI_Viability_Rationale__c), Scope (COMEXI_Retrofit_Type__c, COMEXI_Retrofit_SKU__c, COMEXI_Intervention_Weeks__c, COMEXI_Target_Intervention_Date__c, COMEXI_Down_Payment_Status__c) and Economics (COMEXI_Quote_Total_Cost__c, COMEXI_Quote_Margin_Pct__c, COMEXI_Approval_Required__c), Product2.COMEXI_Target_Margin_Pct__c, Quote.COMEXI_Total_Cost__c / COMEXI_Blended_Margin_Pct__c / COMEXI_Discount_Pct__c plus 25 retrofit fields on Quote grouped in four Details sections - Contexto (COMEXI_Source_Case__c, COMEXI_Target_Asset__c, COMEXI_Machine_Model__c, COMEXI_Retrofit_Type__c, COMEXI_Retrofit_SKU__c, COMEXI_Generated_By_Advisor__c), Intervencion (COMEXI_Intervention_Site__c, COMEXI_Intervention_Country__c, COMEXI_Intervention_Weeks__c, COMEXI_Target_Intervention_Date__c, COMEXI_Intervention_Hours__c, COMEXI_Expenses_Owner__c), Escandallo (COMEXI_Warranty_Uplift_Pct__c, COMEXI_RALF_Pct__c, COMEXI_Country_Risk_Pct__c, COMEXI_Representative_Commission_Pct__c, COMEXI_Representative_Commission_Amount__c, COMEXI_Margin_Amount__c, COMEXI_Margin_Threshold_Pct__c) and Cierre (COMEXI_Approval_Required__c, COMEXI_Down_Payment_Pct__c, COMEXI_Down_Payment_Amount__c, COMEXI_Down_Payment_Status__c, COMEXI_Offer_Version__c, COMEXI_Document_Language__c) and COMEXI_Discount_Effective_Pct__c (formula Discount * 100, needed because a Percent field evaluates as value/100 inside a formula, which makes approval criteria unreadable) and COMEXI_Max_Line_Discount_Pct__c (MAX rollup of QuoteLineItem.Discount, which explains the approval level: Quote.RLM_Approval_Level__c consolidates with MAX the level each line derives from its own discount, so a moderate blended discount can still need two levels), QuoteLineItem.COMEXI_Cost__c / COMEXI_Margin_Pct__c / COMEXI_Target_Margin_Pct__c), 3 Opportunity record types (COMEXI_Comercial, COMEXI_Retrofitting, COMEXI_Servicio) with the COMEXI_Sales_Process business process (the OpportunityType and LeadSource StandardValueSets are extended with the Retrofit and "Caso de servicio" values and surfaced in each record type's picklistValues), three inactive classic approval processes on Quote (COMEXI_Margin_Approval, COMEXI_Discount_Approval_L1, COMEXI_Discount_Approval_L2). They ship deactivated on purpose: quote discount approval now runs on the native Advanced Approvals framework from unpackaged/post_approvals, because a classic approval process neither appears in the Approvals tab nor writes Quote.RLM_Approval_Status__c, which is the field behind the record path. They are kept rather than deleted so the approval history already recorded against them survives. The ProductDiscovery settings, the COMEXI_Admin permission set, the comexiRevenueDashboard LWC (KPI dashboard for the Home page), the comexiCaseRetrofitAdvisor LWC and its COMEXI_RetrofitAdvisorController Apex class (Case record page: case briefing, machine panel, simulated agent run, and real Opportunity + Quote creation via RevSalesTrxn.PlaceSalesTransactionExecutor), the COMEXI_ProductImages static resource (36 branded catalog images consumed by Product2.DisplayUrl), and the COMEXI_MachineryImages static resource (23 real machine photos for the Comexi Maquinaria catalog, consumed by Product2.DisplayUrl). Also surfaces the 16 Account.COMEXI_* fields in four sections on the record Details tab across all three ways the Account detail can render: the Account-Account Layout page layout (used by the standard Account_Record_Page, which renders force:detailPanel from the assigned layout), the Account-RLM Account Layout page layout, and the RLM_Account_Record_Page flexipage (the RLM app's Account override builds its Details tab from explicit flexipage:fieldSection components, so a layout change alone is invisible there). The two layouts and the flexipage override the unpackaged/post_ux versions. Also ships the Case and Asset UI for scene 01 (Case-Case Layout, Asset-Asset Layout, RLM_Case_Record_Page, RLM_Asset_Record_Page) so Case.AssetId, Case.ProductId, Asset.ParentId and the Asset.COMEXI_* fields are visible, and places the comexiCaseRetrofitAdvisor LWC above the tabset on RLM_Case_Record_Page. For the retrofit Opportunity UI it ships the Opportunity-COMEXI Retrofit Layout page layout (assigned to the COMEXI_Retrofitting record type via the Admin profile layoutAssignments) and overrides the shared RLM_Opportunity_Record_Page flexipage to add the four retrofit Details sections (Machine, Viability, Scope, Economics). Because record-type-specific Lightning page assignment is not deployable via metadata in this API version, each retrofit section is gated by a visibilityRule on {!Record.RecordType.DeveloperName} EQUAL COMEXI_Retrofitting, so the sections render only on retrofit opportunities while other record types keep the standard layout. For the Quote it overrides the unpackaged/post_ux Quote-RLM Quote Layout page layout and the RLM_Quote_Record_Page flexipage so the 29 Quote.COMEXI_* fields render in the same four retrofit sections on the Details tab (the Quote Details tab is built from explicit flexipage:fieldSection components, so a layout change alone is invisible there), and places the comexiQuoteApprovalBanner LWC in the Quote header region. That banner is deal guidance, not an approval UI: it reads the blended discount, the highest line discount and the margin live, states how many signatures the offer will cost before the rep promises the discount, and its button starts the native orchestration RLM_Quote_Smart_Approval via COMEXI_QuoteApprovalController. From there the framework owns the cycle - it writes Pending, then Approved or Rejected, into Quote.RLM_Approval_Status__c (the record path), and the signatures are given in the Approvals tab. The Manager, Director and VP steps are assigned to public groups of the same name, so scripts/apex/comexi/16_seed_approver_groups.apex has to put the demo user in all three: otherwise the work item is created but nobody can see it. COMEXI_RetrofitAdvisorController stamps RLM_Payment_Terms__c to Net 30 (leaving that band adds a Finance level to the orchestration) and, on every realign, clears RLM_Approval_Status__c and cancels any in-flight orchestration on the quote, so replaying the demo does not leave two approvals competing for the same offer. The Quote header exposes two Doc Gen entry points, and the flexipage actionNames list is the only thing that decides which: the highlights panel has enableActionsConfiguration true, so it renders that list verbatim and ignores the page layout. Create Proposal is Quote.RLM_Create_Proposal (flow RLM_Quote_Doc_Gen_wAttachments) and Generate Quote Document is Quote.xDO_DocGen_Generate_Quote_Document (flow xDO_DocGen_Generate_Quote_Document, the one with the e-signature toggle). Do not confuse the latter with the standard GenerateQuoteDocument button, which is a different action labelled Generate PDF Document - the list used to carry that one and the Doc Gen action was simply absent. Both are now qualified with the Quote. prefix in the flexipage and, in the page layout, as a QuickAction named Quote.xDO_DocGen_Generate_Quote_Document; the unqualified name fails the deploy. numVisibleActions is 5 so Generate Quote Document, Browse Catalogs, Create Ramp Schedule, Create Proposal and Create Order are buttons and the rest fall into the overflow menu. The e-signature toggle of that modal needs a DocuSign connection the demo org does not have, so leave it at No. Also ships the COMEXI_Propuesta_Retrofit Doc Gen template, which is what the Document Template picklist of both modals offers on the quote. Create Proposal only lists templates that are active with usageType Revenue_Lifecycle_Management, and Generate Quote Document lists every active template regardless of usage type, so the template needs the activate_comexi_docgen_template task afterwards - a deploy always lands it inactive. Neither modal defaults to it: Generate Quote Document preselects QuoteHierarchySalesAgreement, so pick COMEXI_Propuesta_Retrofit by hand on stage. The .docx is built by scripts/build_comexi_retrofit_proposal.py and is deliberately token-free: a seven page branded proposal with every figure of quote 00000049 written in, from the case that originated it to the signature block. Rebuild it with that script whenever the offer is regenerated with different numbers, or the PDF will tell a different story than the screen. It carries nothing the customer should not read - no cost, no margin, no margin threshold, no representative commission and none of the pricing procedure uplifts. What it does say about the internal cycle is that the discount was signed and by whom, which is a selling point; how much margin Comexi keeps is not. The build script asserts this, so keep new rows on the client side of that line. The former COMEXI_RetrofitOffer template is gone from the repo: it was an unedited 3.9 MB copy of RLM_QuoteProposal, and the activation task archives whatever copy of it survives in an org so it stops showing up in the picklist. For the contract leg it ships 20 Contract.COMEXI_* fields in four Details sections - Origen (COMEXI_Source_Case__c, COMEXI_Source_Opportunity__c, COMEXI_Generated_By_Advisor__c, COMEXI_Target_Asset__c, COMEXI_Machine_Model__c), Alcance (COMEXI_Retrofit_Type__c, COMEXI_Retrofit_SKU__c, COMEXI_Obsolete_Component__c), Intervencion (COMEXI_Intervention_Site__c, COMEXI_Intervention_Country__c, COMEXI_Intervention_Weeks__c, COMEXI_Target_Intervention_Date__c, COMEXI_Intervention_Hours__c, COMEXI_Expenses_Owner__c) and Condiciones (COMEXI_Contract_Value__c, COMEXI_Down_Payment_Pct__c, COMEXI_Down_Payment_Amount__c, COMEXI_Payment_Terms__c, COMEXI_Warranty_Months__c, COMEXI_Warranty_Start_Event__c) - and surfaces them in the three places the Contract detail can render. The one that actually matters is Contract-Contract Layout: the ContractLifecycleManagement record type resolves to the standard Contract_Record_Page, which builds its Details tab with force:detailPanel out of the assigned page layout, and that layout is the plain Contract Layout, not the RLM one. Patching only the RLM side left the twenty fields populated but invisible on screen. Regenerate that layout with scripts/apex/comexi/19_add_comexi_sections_to_contract_layout.py, which copies the four COMEXI sections from the RLM layout into a fresh retrieve of the standard one and is safe to re-run. It also ships Contract-RLM Contract Layout and the RLM_Contract_Record_Page flexipage, both overriding unpackaged/post_ux, so the sections are there too if the app is ever pointed at the RLM page; regenerate the flexipage with scripts/apex/comexi/18_add_comexi_fields_to_contract_flexipage.py after any retrieve of the post_ux one. Those fields fill themselves: COMEXI_ContractTrigger fires on any Contract that has SourceQuoteId and is not stamped yet, COMEXI_ContractFromQuoteService copies the scope, the intervention and the terms from the source quote (and the obsolete component from its opportunity, the only place it lives), and COMEXI_ContractDocQueueable generates the contract document out of band and attaches the clause set. It has to listen on update as well as insert: the Create Contract button of the quote saves the contract header first and fills SourceQuoteId in a second step, so an after insert only trigger sees an empty lookup and stamps nothing - the symptom is a Draft contract with twenty blank fields and no queueable job, while the same action over REST does carry the quote in the insert and works, which is what made it look like a code problem. COMEXI_Generated_By_Advisor__c doubles as the done marker so the stamping update does not re-enter, and a static Set in the service covers several updates inside one transaction. It only touches contracts whose quote carries COMEXI_Generated_By_Advisor__c, so contracts from other flows are left alone. The document comes from COMEXI_Contrato_Retrofit, the same eight page branded docx as the proposal plus a Condiciones contractuales page, built by the same script with --variant contract so the two can never drift; it carries usageType Contract_Lifecycle_Management and also needs activate_comexi_docgen_template. The six clauses it prints live in the CLM library as data and ship through provision_comexi_clm_clauses; only their ClauseCatgConfiguration (COMEXI_Retrofit) is metadata and travels here. Signing the contract opens the fulfillment chain. Nineteen Order.COMEXI_* fields carry the retrofit file to whoever has to execute it, grouped in four sections - Origen del pedido (COMEXI_Source_Case__c, COMEXI_Source_Opportunity__c, COMEXI_Target_Asset__c, COMEXI_Machine_Model__c, COMEXI_Generated_By_Advisor__c; QuoteId, ContractId and AccountId are not duplicated because createOrderFromQuote fills the standard ones), Alcance del retrofit, Intervencion and Anticipo y salida a SAP (COMEXI_Down_Payment_Pct__c, COMEXI_Down_Payment_Amount__c, COMEXI_Down_Payment_Status__c, COMEXI_Payment_Terms__c, COMEXI_SAP_Customer_Code__c). Here the page situation is the mirror image of the contract: RLM_Order_Record_Page builds its Details tab with flexipage:fieldSection, so the fields are declared there (scripts/apex/comexi/20_add_comexi_fields_to_order_flexipage.py), and Order-RLM Order Layout is patched as well (scripts/apex/comexi/21_add_comexi_sections_to_order_layout.py) because the UI API reports that layout as the active one. Check which of the two cases you have before assuming either. The automation is a flow and not the trigger, for a hard reason: createOrderFromQuote is exposed to Flow and REST but not to Apex, and calling it with Invocable.Action fails with "createOrderFromQuote isn't a valid action type". COMEXI_Order_From_Signed_Contract fires on Contract Status Signed with an AsyncAfterCommit path so the signing transaction carries none of this, and runs three steps. COMEXI_QuoteRepriceService first: without it the order creation is rejected with "the prices aren't updated. Click Reprice All and try again", because Quote.ValidationResult goes to TransactionIncomplete every time the quote header is touched - and this demo touches it in every scene - while CalculationStatus keeps saying CompletedWithTax and hides the problem. The reprice recipe is the same Place Sales Transaction (PATCH header, pricing FORCE, configuration SKIP) that RLM_PreProcessOrderController already documents for large deals. Then createOrderFromQuote, then COMEXI_OrderFromContractService, which stamps the order and hands it to COMEXI_OrderActivationQueueable. That one activates by updating Status to Activated rather than calling activateOrderAction, which answers "Invalid input for activeOrCartId" for an order born from a quote; the update is what RLM_Submit_Order_on_Activation listens for, and it calls submitOrder and decomposes the order into the fulfillment plan. The decomposition design-time data is not metadata and ships through provision_comexi_dro_retrofit. Note for the demo: the CLM state machine in this org does not allow jumping to Signed. ObjectStateTransition only permits Draft to In Approval Process to Negotiating to Awaiting Signature to Signed, which reads well on stage anyway. Replaying the demo needs no cleanup by hand: resetDemoState now unwinds the whole chain, and the order matters. FulfillmentLineSourceRel goes first or the order refuses to be deleted; the order has to be put back to Draft because an activated order cannot be deleted; the plan goes last and drags its steps, which are never deleted directly because deleting FulfillmentStep in bulk fails with INVALID_QUERY_FILTER_OPERATOR when the platform queries FulfillmentStepDependency with 15 character ids. Flexipage placement of the dashboard is manual via Lightning App Builder.

**Class:** `cumulusci.tasks.salesforce.Deploy`

**Options:**

- `path`: `unpackaged/post_comexi`
...
### `provision_comexi_clm_clauses`

**Description:** Provision the CLM clause library behind the COMEXI retrofit contract: one DocumentClauseSet (Condiciones Retrofit, DefaultLanguage es) with the six clauses the Discovery backs - price and exclusions, payment terms, warranty, installation and commissioning, lead time, confidentiality - and the DocumentAuthoredContent rows that put them in Associated Clauses of COMEXI_Contrato_Retrofit. DocumentClauseSet, DocumentClause and DocumentAuthoredContent are data, not metadata, so they cannot ship in unpackaged/post_comexi and go over REST instead; only the ClauseCatgConfiguration (COMEXI_Retrofit) is deployable and ships with the bundle. Run after activate_comexi_docgen_template - without an active template the clauses are created but not associated. Idempotent, and it refuses to touch a clause set whose DefaultLanguage does not match, because Salesforce blocks that update once created.

**Class:** `tasks.comexi_clm_clauses.ProvisionComexiClmClauses`

---

### `provision_comexi_dro_retrofit`

**Description:** Provision the DRO decomposition behind the COMEXI retrofit order: three fulfillment destination products (COMEXI-DRO-MAT, COMEXI-DRO-OPS, COMEXI-DRO-FIN, all DecompositionScope Order and IsAssetizable false - the platform rejects Order scope on an assetizable product), the DecompositionScope stamp on the sellable products (T100 as Bundle, T100-W11 / T100-TEMP / SRV-MEC / SRV-ELE / SRV-PME as OrderLineItem, the EXP-* deliberately left out because an expense does not decompose), the SAP material codes in Product2.COMEXI_SAP_Material_Code__c, three FulfillmentStepDefinitionGroup (COMEXI Material y SAP, COMEXI Intervencion en planta, COMEXI Finanzas), nine FulfillmentStepDefinition, six FulfillmentStepDependencyDef - the load-bearing one being Arranque de ingenieria depending on Confirmar cobro del anticipo, which is what keeps engineering blocked until the down payment lands - three ProductFulfillmentScenario hanging off the DESTINATION products (not the sellable ones - get that wrong and the plan is born empty and completes instantly), seven ProductFulfillmentDecompRule and the COMEXI Retrofit FulfillmentWorkspace. All of this is data, not metadata, so it cannot ship in unpackaged/post_comexi and goes over REST instead. Run after deploy_post_comexi (it needs COMEXI_SAP_Material_Code__c) and after the COMEXI catalogue is loaded. Idempotent - it checks every record before creating it.

**Class:** `tasks.comexi_dro_retrofit.ProvisionComexiDroRetrofit`
